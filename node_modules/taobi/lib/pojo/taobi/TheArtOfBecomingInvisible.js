 ...
 * @LastEditors: lax
 * @LastEditTime: 2024-09-21 10:08:07
 */
const SOLAR_TERMS = require("./solarTerms");
const {
  Calendar
} = require("tao_calendar");
const TaoConvert = require("./TaoConvert.js");
...
class TheArtOfBecomingInvisible extends TaoConvert {
  ...
  constructor(questionTime, r, arranged, follow = 0, options) {
    ...
    super(options);
    this.follow = this.OPTIONS.follow === undefined ? follow : this.OPTIONS.follow;
    // step1: 根据日期转化干支历
  ...
  #generateElement(e) {
     ...
     */
    // todo test
    let MAO = ~~((this.time.getTime() - this.during[this.solarTerms].getTime()) / (24 * 60 * 60 * 1000) / 5);
    MAO = MAO > 2 ? 2 : MAO;
    // 置润法 暂不使用，也不建议用
    const LEAP = 0;
